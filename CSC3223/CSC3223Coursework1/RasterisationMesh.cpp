...
RasterisationMesh* RasterisationMesh::CreateTriangleFromPoints(const std::vector <Vector3>& inVerts, const std::vector<Vector4>& colours, int type) {
	...
	if (type == 1) {
		for (size_t i = 0; i < inVerts.size() - 2; i += 3) {
			tri->RasterBarycentricTriangle(inVerts[i], inVerts[i + 1], inVerts[i + 2], colours[i], colours[i + 1], colours[i + 2]);
		}
	}
	else if (type == 2) {
		for (size_t i = 0; i < inVerts.size() - 2; i += 3) {
			tri->RasterSpanTriangle(inVerts[i], inVerts[i + 1], inVerts[i + 2], colours[i], colours[i + 1], colours[i + 2]);
		}
	}
	else {
		for (size_t i = 0; i < inVerts.size() - 2; i += 3) {
			tri->RasterLineEquationTriangle(inVerts[i], inVerts[i + 1], inVerts[i + 2], colours[i], colours[i + 1], colours[i + 2]);
		}
	}
...
void RasterisationMesh::RasterSpanTriangle(const Vector3& v0, const Vector3& v1, const Vector3& v2, const Vector4& c0, const Vector4& c1, const Vector4& c2) {
	...
	if (vertices[0]->y == vertices[1]->y) { //make 0 highest
	...
	for (int y = (int)topLeft.y; y < bottomRight.y; ++y) {
		for (int x = (int)topLeft.x; x < bottomRight.x; ++x) {
			...
			if ((int)triSum > abs(triArea)) {
				continue; //and move onto next screen position
			}
